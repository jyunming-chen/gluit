...
glmReadMTL(GLMmodel* model, char* name)
{
  ...
  for (i = 0; i < nummaterials; i++) {
	...
	model->materials[i].texobj = 0 ; // default: no texture id
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'm': case 'M': // map_Kd:
		if (strcmp (buf, "map_Kd") == 0) {
    	fscanf (file, "%s", buf);
		strcpy (texfile, dir);
		strcat (texfile, buf);
		// save the currently bound tex. object
		glGetIntegerv (GL_TEXTURE_BINDING_2D, &tex2dbound);
		
		pngSetStandardOrientation(1);		
    	id = pngBind (texfile, PNG_NOMIPMAP, PNG_SOLID, NULL, GL_CLAMP, GL_NEAREST, GL_NEAREST);

    	if (id > 0) 
			model->materials[nummaterials].texobj = id;
		else
			printf ("texture file [%s] not found...\n", texfile);

		// restore the currently bound tex.object		
 ...
 * scalefactor used.
 * (12/14/2011): so that the vertices of the bounding box are (+/1 1)'s
 * model - properly initialized GLMmodel structure 
 */
GLfloat
...
glmDraw(GLMmodel* model, GLuint mode)
{
  ...
  // IF texture draw is specified, save currently bound texture object (could be 0)
  if (mode & GLM_TEXTURE)
	 glGetIntegerv (GL_TEXTURE_BINDING_2D, &tex2dbound);
  ...
  while (group) {
    if (mode & GLM_MATERIAL) {
	  ...
	  if ((mode & GLM_TEXTURE) && material->texobj) {   // not every group has texture ...
    	glBindTexture (GL_TEXTURE_2D, material->texobj);
	...
	glNormal3fv(&model->normals[3 * triangle->nindices[0]]);
      if ((mode & GLM_TEXTURE) && material->texobj)
